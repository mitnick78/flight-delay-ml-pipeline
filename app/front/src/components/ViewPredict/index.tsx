...
const ViewPredict = ({dataPredict, onclick}: PredictProps) => {
    ...
    return(<div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">

      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Prédiction de vol</h2>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-400">{dataPredict.origin.name_city}</p>
        </div>
        <div>
          <p className="text-xs text-gray-400">Arrivée</p>
          <p className="text-sm font-semibold">{dataPredict.destination.id_city}</p>
          <p className="text-xs text-gray-400">{dataPredict.destination.name_city}</p>
        </div>
      </div>
      ...
      <button
        onClick={onclick}
        className="mt-2 w-full py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-sm text-gray-500 font-medium transition-all"
      >
        Nouvelle prédiction
      </button>
